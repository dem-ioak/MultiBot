...
class Events(commands.Cog):
    ...
    async def on_ready(self):
        for server in self.client.guilds:
            ...
            if "message_id" in board_data and "channel_id" in board_data:
            ...
            if before.channel and before.channel.id != create_id:
                events.append(
                    DataClasses.VCEvent(
                        ...
                        (
                            EventType.LEAVE_VC
                            if before.channel.id != afk_corner
                            else EventType.LEAVE_AFK
                        ),
                        before.channel.id,
                    )
                )
            ...
            if after.channel:
                ...
                if created_vc:
                    ...
                    category = discord.utils.get(
                        member.guild.categories, id=category_id
                    )
                    created_channel = await member.guild.create_voice_channel(
                        name=f"{member.name}'s VC", category=category
                    )
                    ...
                    events.append(
                        DataClasses.VCEvent(
                            ...
                            (
                                EventType.JOIN_VC
                                if after.channel.id != afk_corner
                                else EventType.JOIN_AFK
                            ),
                            after.channel.id,
                        )
                    )
    ...
    async def on_message(self, message):
        ...
        if server_data["levels"]:
            ...
            if not rate_limit:
                ...
                USERS.update_one(
                    user_data, {"$set": {"xp": updated_xp, "level": current_level}}
                )
